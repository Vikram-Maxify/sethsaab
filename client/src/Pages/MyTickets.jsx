...
// Priority:
// 1. item.drawDate
// 2. item.entry.drawDate
// 3. item.config.drawDate
// 4. item.lotteryConfig.drawDate
//
// IMPORTANT:
// Draw date is NOT taken from entryDate.
// =====================================================
...
const formatDrawDate = (item) => {
  const drawDateValue =
    item?.drawDate ||
    item?.entry?.drawDate ||
    item?.config?.drawDate ||
    item?.lotteryConfig?.drawDate;

  if (drawDateValue) {
    const date = new Date(drawDateValue);
    ...
    if (!Number.isNaN(date.getTime())) {
      ...
      const month = date.toLocaleDateString("hi-IN", {
        timeZone: "Asia/Kolkata",
        month: "long",
      });
  ...
  // ===================================================
  // FALLBACK
  // If API still sends date/month/year
  // ===================================================
...
const MyTickets = () => {
  ...
  const tickets = useMemo(() => {
    ...
    return entries.map((item, index) => {
      ...
      //
      // IMPORTANT:
      // This uses drawDate, NOT entryDate.
      // =================================================
      ...
      //
      // IMPORTANT:
      // createdAt = actual purchase date/time
      // entryDate is NOT used.
      // =================================================
      ...
      //
      // IMPORTANT:
      // createdAt = actual purchase time
      // =================================================
      ...
      return {
        ...
        // Purchase date from createdAt
        purchaseDate,

        // Purchase time from createdAt
        purchaseTime,
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`h-[47px] rounded-[10px] border flex items-center justify-center whitespace-nowrap transition-all ${
                  active
                    ? "border-[#ffe27a] text-black font-extrabold bg-gradient-to-b from-[#ffe77b] via-[#f5c542] to-[#e9b528] shadow-[0_3px_12px_rgba(245,197,66,0.22)]"
                    : "border-[#353938] bg-[#101212] text-[#eeeeee] font-semibold"
                }`}
              >
