...
const BuyTicket = () => {
  ...
  const [pendingLotteryNumber, setPendingLotteryNumber] = useState("");
  ...
  // ===================================================
  // PURCHASE TICKET (OPEN UPI MODAL)
  // ===================================================
  ...
  const handlePurchase = () => {
    ...
    if (!ticketPriceFromApi || Number(ticketPriceFromApi) <= 0) {
      setLocalError("टिकट की कीमत उपलब्ध नहीं है");
      return;
    }

    // SAVE NUMBER & OPEN UPI MODAL
    setPendingLotteryNumber(lotteryNumber);
    setShowUpiModal(true);
  };

  // ===================================================
  // AFTER UPI PAYMENT SUCCESS → BUY TICKET
  // ===================================================
  ...
  const handlePaymentSuccess = async () => {
    try {
      const result = await dispatch(
        addUserLotteryEntry({
          number: pendingLotteryNumber,
          amount: Number(TICKET_PRICE),
        }),
      ).unwrap();

      setLocalSuccess(
        result?.message || "आपका टिकट सफलतापूर्वक बुक हो गया",
      );

      setNumbers(["", "", "", "", "", ""]);
      setPendingLotteryNumber("");
      setShowUpiModal(false);
    } catch (err) {
      setLocalError(
        typeof err === "string"
          ? err
          : err?.message || "टिकट खरीदने में समस्या हुई",
      );
    }
  };
        ...
        <section className="mt-3 rounded-[18px] border border-[#353535] bg-[#080a0a] p-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              disabled={purchaseLoading || activeLoading || amountLoading}
          ...
          <div className="grid grid-cols-6 gap-[7px] mt-4">
            {numbers.map((number, index) => (
              <input
                ...
                disabled={purchaseLoading || activeLoading}
